 ...
 * Revision;
 *
 * 01/31/03 - first revision. 05/21/03 - Giordano Sassaroli
 * <sassarol@cefriel.it> - Description: inserted a check at the beginning of the
 * setService method - Problem : If the EventSubURL does not start with a '/',
 * the device could refuse event subscription - Error : it is not an error, but
 * adding the '/' when missing allows the integration with the Intel devices
 * 09/02/03 - Giordano Sassaroli <sassarol@cefriel.it> - Problem :
 ...
 * setRequestHost/setService should be changed as follows 06/11/04 - Markus
 * Thurner <markus.thurner@fh-hagenberg.at> (06/11/2004) - Changed setServie()
 * to get the host address from the SSDP Location field when the URLBase is
 * null. 12/06/04 - Grzegorz Lehmann <grzegorz.lehmann@dai-labor.de> - Stefano
 * Lenzi <kismet-sl@users.sourceforge.net> - Fixed getSID() to loop between
 * getSID() and hasSID();
 *
 ********************************************************************/
