 ...
 * Revision;
 *
 * 11/18/02 - first revision. 05/13/03 - Added support for IPv6 and loopback
 * address. 12/26/03 - Added support for XML Parser 06/18/03 - Added INMPR03 and
 * INMPR03_VERSION. 04/14/06 - Added some functios about time-to-live, and the
 * default value is 4. 05/11/09 - Changed loadDefaultXMLParser() to load
 * org.cybergarage.xml.parser.XmlPullParser at first.
 *
 ******************************************************************/
...
public class UPnP
{
     ...
     * First if present loads the parsers specified by the system property
     * {@link UPnP#XML_CLASS_PROPERTTY}<br>
     * - Second by a fall-back technique, it tries to load the XMLParser from
     * one<br>
     * of the following classes: {@link JaxpParser}, {@link kXML2Parser},
     * {@link XercesParser}
     *
     * @return {@link Parser} which has been loaded successuflly or null
     *         otherwise
     *
     * @since 1.8.0
     */
    private static Parser loadDefaultXMLParser()
    {
        ...
        String[] parserClass = new String[]{System.getProperty(XML_CLASS_PROPERTTY),
        ...
        for (int i = 0; i < parserClass.length; i++)
        {
            ...
            {
                Debug.warning("Unable to load " + parserClass[i]
                    + " as XMLParser due to " + e);
            }
        }
        return null;
    }
