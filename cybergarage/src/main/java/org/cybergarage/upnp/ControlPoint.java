 ...
 * Revision:
 *
 * 11/18/02 - first revision. 05/13/03 - Changed to create socket threads each
 * local interfaces. (HTTP, SSDPNotiry, SSDPSerachResponse) 05/28/03 - Changed
 * to send m-serach packets from SSDPSearchResponseSocket. The socket doesn't
 * bind interface address. - SSDPSearchResponsSocketList that binds a port and a
 * interface can't send m-serch packets of IPv6 on J2SE v 1.4.1_02 and Redhat 9.
 * 07/23/03 - Suzan Foster (suislief) - Fixed a bug. HOST field was missing.
 * 07/29/03 - Synchronized when a device is added by the ssdp message. 09/08/03
 * - Giordano Sassaroli <sassarol@cefriel.it> - Problem : when an event
 * notification message is received and the message contains updates on more
 * than one variable, only the first variable update is notified. - Error : the
 * other xml nodes of the message are ignored - Fix : add two methods to the
 * NotifyRequest for extracting the property array and modify the
 * httpRequestReceived method in ControlPoint 12/12/03 - Added a static() to
 * initialize UPnP class. 01/06/04 - Added the following methods to remove
 * expired devices automatically removeExpiredDevices()
 * setExpiredDeviceMonitoringInterval()/getExpiredDeviceMonitoringInterval()
 * setDeviceDisposer()/getDeviceDisposer() 04/20/04 - Added the following
 * methods. start(String target, int mx) and start(String target). 06/23/04 -
 * Added setNMPRMode() and isNMPRMode(). 07/08/04 - Added
 * renewSubscriberService(). - Changed start() to create renew subscriber thread
 * when the NMPR mode is true. 08/17/04 - Fixed removeExpiredDevices() to remove
 * using the device array. 10/16/04 - Oliver Newell <newell@media-rush.com> -
 * Added this class to allow ControlPoint applications to be notified when the
 * ControlPoint base class adds/removes a UPnP device 03/30/05 - Changed
 * addDevice() to use Parser::parse(URL). 04/12/06 - Added setUserData() and
 * getUserData() to set a user original data object.
 *
 *******************************************************************/
...
public class ControlPoint implements HTTPRequestListener
{
    ...
    ////////////////////////////////////////////////
    //	SSDPPacket 接收到DLNA设备发送的Notify消息
    ////////////////////////////////////////////////
    ...
    /**
     * Control接收到DLNA设备对M-Search消息的响应
     *
     * @param packet
     */
    ...
    public boolean start(String target, int mx)
    {
        ...
        ////////////////////////////////////////
        // Notify Socket 开启接受DLNA设备发送的Notify消息
        ////////////////////////////////////////
        ...
        ////////////////////////////////////////
        // SeachResponse Socket 开启接受DLNA设备对ControlPoint发送的M-Search消息的响应
        ////////////////////////////////////////
        ...
        ////////////////////////////////////////
        // search root devices ControlPoint开始发送M-Search消息，搜索设备
        ////////////////////////////////////////
