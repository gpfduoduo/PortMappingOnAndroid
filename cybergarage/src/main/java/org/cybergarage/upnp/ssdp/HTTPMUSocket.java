 ...
 * Revision;
 *
 * 11/18/02 - first revision. 09/03/03 - Changed to open the socket using
 * setReuseAddress(). 12/10/03 - Fixed getLocalAddress() to return a valid
 * interface address. 02/28/04 - Added getMulticastInetAddress(),
 * getMulticastAddress(). 11/19/04 - Theo Beisch <theo.beisch@gmx.de> - Changed
 * send() to set the TTL as 4. 08/23/07 - Thanks for Kazuyuki Shudo - Changed
 * receive() to throw IOException. 01/10/08 - Changed getLocalAddress() to
 * return a brank string when the ssdpMultiGroup or ssdpMultiIf is null on
 * Android m3-rc37a.
 *
 ******************************************************************/
...
public class HTTPMUSocket
{
    ...
    /**
     * @param addr {@link String} rappresenting the multicast hostname to join
     *            into.
     * @param port int rappresenting the port to be use poth as source and
     *            destination
     * @param bindAddr {@link InetAddress} which identify the hostname of the
     *            interface to use for sending and recieving multicast packet
     */
