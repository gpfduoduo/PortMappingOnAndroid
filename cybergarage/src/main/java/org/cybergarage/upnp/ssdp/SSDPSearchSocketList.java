 ...
 * Revision;
 *
 * 05/08/03 - first revision. 05/28/03 - Moved post() for SSDPSearchRequest to
 * SSDPResponseSocket. - Removed open(int).
 *
 ******************************************************************/
...
public class SSDPSearchSocketList extends Vector
{
     ...
     * @param multicastIPv4 The IPv4 address that we will used for multicast
     *            comunication
     * @param multicastIPv6 The IPv6 address that we will used for multicast
     *            comunication
     * @since 1.8
     */
        ...
        for (int i = 0; i < bindAddresses.length; i++)
        {
            if (bindAddresses[i] != null)
            {
                ...
                if (HostInterface.isIPv6Address(bindAddresses[i]))
                    ssdpSearchSocket = new SSDPSearchSocket(bindAddresses[i], port,
                        multicastIPv6);
                else
                    ssdpSearchSocket = new SSDPSearchSocket(bindAddresses[i], port,
                        multicastIPv4);
