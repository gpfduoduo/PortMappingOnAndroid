 ...
 * Revision;
 *
 * 11/20/02 - first revision. 05/13/03 - Added support for IPv6. 02/20/04 - Inma
 * Marin Lopez <inma@dif.um.es> - Added a multicast filter using the SSDP
 * pakcet. 04/20/05 - Mikael Hakman <mhakman@dkab.net> - Handle receive()
 * returning null. - Added close() in stop(). 08/23/07 - Thanks for Kazuyuki
 * Shudo - Changed run() to catch IOException of HTTPMUSocket::receive().
 * 01/31/08 - Changed start() not to abort when the interface infomation is null
 * on Android m3-rc37a.
 *
 ******************************************************************/
...
public class SSDPNotifySocket extends HTTPMUSocket implements Runnable
{
    ...
    /**
     * This method send a {@link SSDPNotifyRequest} over
     * {@link SSDPNotifySocket}
     *
    ...
    public void run()
    {
        ...
        while (deviceNotifyThread == thisThread)
        {
            ...
            {
                Debug.warning("Invalidate Multicast Recieved from IP " + maddr + " on "
                    + pmaddr);
                continue;
            }
