 ...
 * Revision;
 *
 * 12/30/02 - first revision. 05/13/03 - Added support for IPv6. 05/28/03 -
 * Moved post() for SSDPSearchRequest to SSDPResponseSocketList. 04/20/05 -
 * Mikael Hakman <mhakman@dkab.net> - Added close() in stop(). - Added test for
 * null return from receive() in run(). 08/23/07 - Thanks for Kazuyuki Shudo -
 * Changed run() to catch IOException of HTTPMUSocket::receive(). 01/10/08 -
...
public class SSDPSearchSocket extends HTTPMUSocket implements Runnable
{
    ...
    {
        if ((HostInterface.isIPv6Address(bind))
            && (HostInterface.isIPv6Address(multicast)))
        {
            useIPv6Address = true;
        }
        else if (HostInterface.isIPv4Address(bind)
            && (HostInterface.isIPv4Address(multicast)))
        {
            useIPv6Address = false;
        }
        else
    ...
    /**
     *
     * @param bindAddr the hostname of the interface to use for senfing
     *            multicast packet
     * @return true if and only if it open the socket
     * @see {@link SSDP} for default multicast and port destination of the
     *      packtes
     */
