 ...
 * Revision:
 *
 * 10/22/03 - first revision. 03/02/04 - Fixed a bug when update() is executed
 * because the root node's ContentDirectory is null. 03/12/04 - Thanks for
 * Robert Johansson <robert.johansson@kreatel.se> - I ran into the problem that
 * the system can not send big files. It uses FileUtil.Load to load files. If
 * the file is too big we get an OutOfMemory exception. 04/05/04 - Added
 * getFormatMimeTypes(), Deleted getFormat() and getNFormats() for C++ porting.
 * 04/27/04 - Changed getContentExportURL() usint the string ID. 06/20/04 -
 * Changed contentExportRequestRecieved() to set the ConnectionInfo. 08/07/04 -
...
public class ContentDirectory extends ThreadCore implements ActionListener, QueryListener
{
    ...
    /*
     * public String []getFormatMimeTypes() { int formatCnt = formatList.size();
     * String mimeType[] = new String[formatCnt]; for (int n=0; n<formatCnt;
     * n++) { Format format = formatList.getFormat(n); mimeType[n] =
     * format.getMimeType(); } return mimeType; }
     */
    ...
    private void sortContentNodeList(ContentNode conNode[], SortCap sortCap,
            boolean ascSeq)
    {
        // Selection Sort
    ...
    private ContentNodeList sortContentNodeList(ContentNodeList contentNodeList,
            String sortCriteria)
    {
    ...
    private boolean browseDirectChildrenActionReceived(BrowseAction action)
    {
        ...
        ContentNodeList sortedContentNodeList = sortContentNodeList(contentNodeList,
            sortCriteria);
    ...
    private int getSearchContentList(ContainerNode node,
            SearchCriteriaList searchCriList, SearchCapList searchCapList,
            ContentNodeList contentNodeList)
    {
        ...
        for (int n = 0; n < nChildNodes; n++)
        {
            ...
            if (cnode.isContainerNode())
                getSearchContentList((ContainerNode) cnode, searchCriList, searchCapList,
                    contentNodeList);
        }
    ...
    {
        return "http://" + getInterfaceAddress() + ":" + getHTTPPort()
            + CONTENT_EXPORT_URI + "?" + CONTENT_ID + "=" + id;
    }

    public String getContentImportURL(String id)
    {
        return "http://" + getInterfaceAddress() + ":" + getHTTPPort()
            + CONTENT_IMPORT_URI + "?" + CONTENT_ID + "=" + id;
    }
