 ...
 * Revision;
 *
 * 12/12/02 - first revision. 03/11/04 - Added the following methods about chunk
 * size. setChunkSize(), getChunkSize(). 08/26/04 - Added a isOnlyHeader to
 * post(). 03/02/05 - Changed post() to suppot chunked stream. 06/10/05 -
 * Changed post() to add a Date headedr to the HTTPResponse before the posting.
 * 07/07/05 - Lee Peik Feng <pflee@users.sourceforge.net> - Fixed post() to
 * output the chunk size as a hex string.
 *
 ******************************************************************/
...
public class HTTPSocket
{
            ...
            boolean isOnlyHeader)
    {
        ...
        if (httpRes.hasContentInputStream() == true)
            return post(httpRes, httpRes.getContentInputStream(), contentOffset,
                contentLength, isOnlyHeader);
        return post(httpRes, httpRes.getContent(), contentOffset, contentLength,
            isOnlyHeader);
    }
}
