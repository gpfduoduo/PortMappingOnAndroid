 ...
 * Revision;
 *
 * 11/19/02 - first revision. 05/26/04 - Jan Newmarch
 * <jan.newmarch@infotech.monash.edu.au> (05/26/04) - Fixed getValue() to
 * compare using String::equals() instead of String::startWidth().
 *
 ******************************************************************/
...
public class HTTPHeader
{
    ...
    public HTTPHeader(String lineStr)
    {
        ...
        String value = new String(lineStr.getBytes(), colonIdx + 1, lineStr.length()
            - colonIdx - 1);
        setName(name.trim());
        setValue(value.trim());
    }
    ...
    public final static String getValue(String data, String name)
    {
        ...
        LineNumberReader lineReader = new LineNumberReader(strReader, Math.min(
